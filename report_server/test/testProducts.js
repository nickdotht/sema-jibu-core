...
describe('Testing Products API', function() {
			...
			authenticate(server).then(function(token) {
				chai.request(server)
					...
					.end(function(err, res) {
						...
						expect(res.body.products[0].description).to.be.equal(
							'Description Product-1'
						);
						expect(res.body.products[0].updatedDate).to.be.equal(
							'2018-01-01T08:00:00.000Z'
						);
						expect(res.body.products[0].priceCurrency).to.be.equal(
							'USD'
						);
						expect(res.body.products[0].priceAmount).to.be.equal(
							5.0
						);
						expect(res.body.products[0].unitPerProduct).to.be.equal(
							1
						);
						expect(res.body.products[0].unitMeasure).to.be.equal(
							'tons'
						);
						expect(res.body.products[0].cogsAmount).to.be.equal(
							4.0
						);
	...
	describe('GET Should return product mrp info on product/kiosk/salesChannel', function() {
		it('Should pass, returning all 5 product mrps', done => {
			authenticate(server).then(function(token) {
				findKioskId
					.findKioskId(server, token, 'UnitTestCustomers')
					.then(function(kiosk) {
						findProductId
							.findProductId(server, token, 'sku1')
							.then(function(product) {
								let url = sprintf(
									'/sema/site/product-mrps?site-id=%d',
									kiosk.id
								);
								chai.request(server)
									.get(url)
									.set('Authorization', token)
									.end(function(err, res) {
										expect(res.body.productMRPs).to.be.an(
											'array'
										);
										expect(
											res.body.productMRPs.length
										).to.be.equal(6);
										res.should.have.status(200);
										for (
											let index = 0;
											index < res.body.productMRPs.length;
											index++
										) {
											if (
												res.body.productMRPs[index]
				...
				findKioskId
					...
					.then(function(kiosk) {
						findProductId
							...
							.then(function(product) {
								let url = sprintf(
									'/sema/site/product-mrps?site-id=%d&updated-date=%s',
									kiosk.id,
									new Date('2017-1-1')
								);
								chai.request(server)
