...
def importHaitDb( dbPopulate, dbReadHaiti):
    ...
    regions = dbReadHaiti.read_regions()
    for region in regions:
        dbPopulate.populate_region( region[0], region[1], "description")

    kiosks = dbReadHaiti.read_kiosks()
    for kiosk in kiosks:
        dbPopulate.populate_kiosk( kiosk[0], kiosk[1])
